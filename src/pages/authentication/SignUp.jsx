...
const SignUp = () => {
  ...
  const handleSignUp = async (values, resetForm) => {
    ...
    try {
      ...
      dispatch(loginSuccess({ user: { uid: userCredential.user.uid, email: userCredential.user.email } }));
      resetForm();
      navigate('/dashboard');
    }
    catch (error) {
      alert(error.message.includes('email-already-in-use') ? 'Email already exists. Sign in instead.' : 'An error occurred. Please try again.');
            ...
            <SectionRight>
              ...
              {({ errors, values, handleChange }) => (
                <Form>
                  ...
                  <FormField>
                    <GoogleSignIn type="button" onClick={logGoogleUser}><GoogleIcon src={gIcon} width={16} height={16}/>Continue with Google</GoogleSignIn>
                  </FormField>
                  <FormField>
                    <Text>By signing up, you agree to our <Link>Terms of Service</Link> and <Link>Privacy Policy</Link>.</Text>
                    <Text style={{ marginTop: "0.5rem" }}>Already have an account? <Link style={{ color: colors.accent2 }} onClick={() => navigate('/signin')}>Sign In</Link></Text>
                  </FormField>
                </Form>
              )}
              </Formik>
